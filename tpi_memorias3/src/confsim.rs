...
impl ConfiguracionSimulacion {
    ...
    pub fn validar_configuracion(&self) -> Result<(), String> {
        ...
        // Validar que la estrategia esté entre las opciones válidas
        let estrategias_validas = ["first-fit", "best-fit", "next-fit", "worst-fit"];
        if !estrategias_validas.contains(&self.estrategia.as_str()) {
            return Err(format!(
                "Estrategia no válida: {}. Las estrategias válidas son: {:?}",
                self.estrategia, estrategias_validas
            ));
        }
